...
public function getStatistics()
{
    ...
    $prixNonAnomalies = Consommations::where('anomalie', false)->sum('mt_sub');
    ...
    $monthlyConsumptions = Consommations::selectRaw('MONTH(date) as month, SUM(qte) as total_quantity')
        ...
        ->toArray();
    // Monthly consumption data
    $monthlyAnomalies = Consommations::selectRaw('MONTH(date) as month, SUM(qte) as total_quantity')
        ->where('anomalie', true)
        ->groupBy('month')
        ->orderBy('month')
        ->pluck('total_quantity', 'month')
    ...
    // Ensure all months are represented
    for ($i = 1; $i <= 12; $i++) {
        if (!isset($monthlyAnomalies[$i])) {
            $monthlyAnomalies[$i] = 0;
        }
    }

    ksort($monthlyConsumptions);
    ksort($monthlyAnomalies);
    ...
    return response()->json([
        ...
        'prixNonAnomalies' => $prixNonAnomalies,
        'monthly_data' => array_values($monthlyConsumptions), // Ordered data for the chart
        'monthly_data_anomalies' => array_values($monthlyAnomalies), // Ordered data for the chart
    ]);
}



public function getMonthlyAnomaliesForMonth($year)
{
    try {
        $rst=[];
        for ($i=1; $i <13 ; $i++) {
...
public function clearMonthlyConsumption($month)
{
    $consumptions = Consommations::whereMonth('date', $month)
        ->get();

    for ($i=0; $i < count($consumptions) ; $i++) {
        $consumptions[$i]->delete();
    }


    return response()->json([
        'payload' => 'consumptions has been deleted successfully',
        'status' => 200,
    ]);
}
